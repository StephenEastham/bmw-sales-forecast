...
flowchart TD
  ...
  subgraph AnalysisActions["L4: Analysis substeps"]
    direction TB
    YearAgg["groupby Year to Total_Sales"]
  ...
  subgraph ReportingActions["L4: Reporting substeps"]
    direction TB
    FormatReport["format strings and f-strings"]
    TopModels["compute top 5 models"]
    ByRegion["compute totals and percentages"]
    WriteFileMonthly["write sales_report_timestamp.txt"]
  ...
  subgraph AggActions["L4: Aggregator substeps"]
    direction TB
    CollectPNGs["list OUTPUT_DIR pngs"]
    CollectHTML["list OUTPUT_DIR htmls"]
