...
def create_interactive_dashboard(ts_years, ts_data, future_years, future_values, 
    ...
    # Only add forecast trace if future data is provided and valid
    try:
        has_forecast = future_years is not None and future_values is not None and len(future_years) == len(future_values) and len(future_years) > 0
    except Exception:
        has_forecast = False
    ...
    if has_forecast:
        ...
        # If no forecast, add a lightweight annotation instead
        fig_forecast.add_annotation(text='Forecasting disabled', xref='paper', yref='paper', x=0.75, y=0.95,
