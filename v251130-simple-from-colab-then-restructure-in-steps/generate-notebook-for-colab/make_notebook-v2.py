...
def make_notebook(order: List[Path], output: Path):
    ...
    for p in order:
        ...
        module_name = p.stem
        # Use ensure_ascii=False to prevent json.dumps from outputting surrogate pairs
        # for non-BMP characters (like emojis), which Python's string literal parser
        # would interpret as two separate surrogate characters instead of one unicode char,
        # causing UnicodeEncodeError during exec().
        src_json = json.dumps(text, ensure_ascii=False)
        module_init_code = (
