...
Purpose
-------
This document explains the structure and behavior of `make_notebook.py` (located beside this file). The script combines multiple Python modules into a single self-contained Jupyter notebook. It embeds module source code directly into notebook cells using a "shim" pattern to support imports without needing external `.py` files. It also copies CSV files from the source folder and performs a cleanup of the notebook directory before generation.
...
7. Call `make_notebook(order, output)` to create the notebook with embedded module shims.
...
- `make_notebook(order: List[Path], output: Path)`
  - Core notebook generation function. For each file in `order`:
    ...
    - Stores processed text in a dictionary.
  - Copies any `*.csv` files from the source directory to the notebook directory.
  - Builds the notebook JSON structure in memory:
    - An initialization code cell.
    - For each module:
      - Optional markdown cells for module docstrings.
      - A code cell containing a "shim": it creates a `types.ModuleType`, executes the module source (embedded as a JSON string) into it, and registers it in `sys.modules`. This allows subsequent cells to `import` this module.
  - Writes the `.ipynb` file to `output`.

Design notes and rationale
-------------------------
- In-memory Module Shims:
  - To ensure the notebook runs in environments like Google Colab without requiring users to upload multiple `.py` files, the script embeds source code directly.
  - It uses `json.dumps(src, ensure_ascii=False)` to safely embed code (including emojis) as string literals.
  - It uses `exec()` and `sys.modules` to simulate standard Python modules, so `import` statements work normally.
...
- Imports failing inside the notebook: ensure you have run the cells in order. The module shim cells must be executed to register the modules in `sys.modules` before other cells can import them.
