...
Overview:
- Purpose: Combine multiple `.py` modules into one `.ipynb` file, write module `.py` files next to the notebook, and copy any CSV data files from the source folder.
...
Behavior Notes:
- The script writes processed `.py` module files into the notebook directory (so the notebook kernel can `import` them).
...
- If imports fail inside the notebook, ensure the generated `.py` module files are present next to the `.ipynb` and that the notebook's working directory contains them.
