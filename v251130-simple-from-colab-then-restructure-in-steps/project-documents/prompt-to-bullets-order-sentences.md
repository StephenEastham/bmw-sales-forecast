...
Task #1
Converted step{step-number}/detailed-tree-step{step-number}-format2.md into the nested markdown bullet style. This keeps the summary-before-tree requirement and uses indented Markdown bullets to show the same runtime notes I have in the ASCII version.
...
In the file, correct the section that uses nested markdown bullets. Rewrite the nested bullet section at the top of detailed-tree-step{step-number}-format2.md so that each sentence is shorter, ordered by runtime.
