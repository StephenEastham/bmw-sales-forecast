...
This example is not really clear. The sentence is too long. It should be in multiple parts, but you decide on the parts. Also, logically, the order of items in the sentence is the wrong way round. You must express the order so that clearly matches the order in which the system runs the items at runtime .
